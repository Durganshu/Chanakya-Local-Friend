...
class CustomReActSingleInputOutputParser(AgentOutputParser):
    ...
    def parse(self, text: str) -> Union[AgentAction, AgentFinish]:
        ...
        outer_think_match = re.fullmatch(
            r"<think>(.*?)</think>",
        ...
        if outer_think_match:
            ...
            app.logger.debug(
                "ReAct Parser: No single outer <think>...</think> block found. Parsing original text."
            )
            # This parser assumes the PRIMARY ReAct structure is *inside* the optional outer <think> block.

        app.logger.debug(
            f"ReAct Parser: Content to parse for Action/Final Answer:\n'''{content_to_parse}'''"
        )
        ...
        if not content_to_parse:  # Empty after stripping outer <think>
            ...
            raise OutputParserException(
                error_msg, observation=error_msg, llm_output=original_cleaned_text
            )

        if (
            content_to_parse.lower() == "<think>"
        ):  # Literal "<think>" string inside the outer wrapper (or as whole output)
        ...
        content_for_keywords = re.sub(
            r"<think>.*?</think>", "", content_to_parse, flags=re.DOTALL | re.IGNORECASE
        ).strip()
        app.logger.debug(
            f"ReAct Parser: Content for keyword search (after all think removals):\n'''{content_for_keywords}'''"
        )
        ...
        app.logger.debug(
            f"ReAct Parser: Keyword search: includes_answer={includes_answer}, includes_action={includes_action}"
        )
        ...
        if includes_answer and includes_action:
            ...
            if idx_answer > idx_action:
                app.logger.info(
                    "ReAct Parser: Both Action and Final Answer found; Final Answer is later, choosing Final Answer."
                )
                ...
                app.logger.warning(
                    f"ReAct Parser: Both Action and Final Answer found; Action is later. Attempting Action. Original: `{original_cleaned_text}`"
                )
                # Fall through
            ...
            if (
                ...
                if (
                    ...
                    action_input_str_for_json = (
                        ...
                        if action_input_str_raw.startswith('"')
                        else temp_unquoted.replace("\\'", "'")
                    )
        ...
        if content_for_keywords:
            app.logger.info(
                f"ReAct Parser: No Action or Final Answer keywords found. Returning remaining content as output: '{content_for_keywords}'. Original: `{original_cleaned_text}`"
            )
        ...
        raise OutputParserException(
            final_error_msg,
            observation=final_error_msg,
            llm_output=original_cleaned_text,
        )
...
def get_chanakya_react_agent_with_history():
    ...
    try:
        ...
        app.logger.warning(
            "'tool_specific_instructions.txt' not found. Proceeding without tool instructions."
        )
