...
if not APP_SECRET_KEY:
    if DEBUG_MODE:
        ...
        print(f"[CONFIG WARNING] APP_SECRET_KEY not set. Using generated dev fallback: {APP_SECRET_KEY}")
    else:
        raise ValueError("APP_SECRET_KEY environment variable is required in production mode (FLASK_DEBUG is False).")
...
if llm_num_ctx_small_env is not None:
    ...
    if llm_num_ctx_small_env:
        try:
            ...
            print(f"[CONFIG WARNING] Invalid LLM_NUM_CTX_SMALL: '{llm_num_ctx_small_env}', using 2048")
